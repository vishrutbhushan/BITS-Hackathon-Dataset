...
from exact_math import mean_median_gap, percentage, rounded_average
        ...
        for task in plan.subtasks:
            ...
            # 2. Relational SQL Queries
                ...
                if sql_type == "ar_outstanding":
                    ...
                    if vals:
                        mode = task.query_params.get("difference_mode", "absolute")
                        ...
                        diff = mean_median_gap(vals, mode=mode)
                    ...
                    avg_val = rounded_average(r["val_inr"] for r in rows)
                ...
                elif sql_type == "client_grade_projects":
                    client = self._resolve_client(
                        task.query_params.get("client") or plan.anchor_client,
                        task.query_params.get("pkg_num") or plan.anchor_package_num,
                        task.query_params.get("person") or plan.anchor_person,
                    )
                    grade = task.query_params.get("grade")
                    comparison = task.query_params.get("comparison", "exact")
                    grade_order = ("Satisfactory", "Good", "Very Good", "Excellent", "Outstanding")
                    if grade not in grade_order:
                        raise ValueError(f"Unknown performance grade: {grade!r}")
                    index = grade_order.index(grade)
                    if comparison == "gte":
                        accepted_grades = grade_order[index:]
                    elif comparison == "gt":
                        accepted_grades = grade_order[index + 1:]
                    else:
                        accepted_grades = (grade,)
                    if not accepted_grades:
                        rows = []
                    else:
                        placeholders = ", ".join("?" for _ in accepted_grades)
                        df = self.db.fetchall(
                            f"""
                            SELECT project_id, title, performance_grading, contract_value_inr
                            FROM projects
                            WHERE canonical_client = ?
                              AND performance_grading IN ({placeholders})
                            """,
                            [client, *accepted_grades],
                        )
                        rows = [
                            {"project_id": r[0], "title": r[1], "grade": r[2], "val_inr": r[3]}
                            for r in df
                        ]
                    res.sql_rows = rows
                    res.computed_value = sum(row["val_inr"] for row in rows)
                    res.summary = (
                        f"Client '{client}' has {len(rows)} works matching "
                        f"grade boundary {comparison} {grade}, sum: {res.computed_value} INR."
                    )
                    ...
                    comparison = task.query_params.get("comparison", "gte")
                    operator = ">" if comparison == "gt" else ">="
                    sql = f"""
                        SELECT project_id, title, contract_value_inr
                        FROM projects
                        WHERE canonical_client = ? AND contract_value_inr {operator} ?
                    """
                    ...
                    total_val = sum(r["val_inr"] for r in rows)
                    res.computed_value = total_val
                    res.summary = f"Client '{client}' has {len(rows)} works {operator} {thresh} INR, sum: {total_val} INR."
                    ...
                    growth = percentage(values[year2] - values[year1], values[year1])
                    res.sql_rows = [
                    ...
                    variance = Decimal(str(measured)) - Decimal(str(tender_quantity))
                    ...
                    res.computed_value = float(variance.quantize(Decimal("0.001")))
            ...
            # 3. Math Computation
                ...
                if metric == "date_diff_days":
                    ...
                    if source_rows:
                        ...
                        avg = rounded_average(r["val_inr"] for r in source_rows)
