...
from entity_resolver import EntityResolver, contains_phrase, normalize_text
...
PERFORMANCE_GRADES = ("Outstanding", "Excellent", "Very Good", "Good", "Satisfactory")
...
def extract_threshold_inr(text: str) -> Optional[int]:
    ...
    if numeric:
        ...
        try:
            amount = Decimal(numeric.group(1)) * multiplier
...
def extract_threshold_comparison(text: str) -> str:
    """Return the mathematically explicit boundary for a threshold phrase."""
    normalized = normalize_text(text)
    if re.search(r"\b(?:at least|minimum|no less than|not less than)\b", normalized) or ">=" in text:
        return "gte"
    if re.search(r"\b(?:above|greater than|more than|over|exceeding)\b", normalized) or re.search(r"(?<![<>=])>(?!=)", text):
        return "gt"
    # Tender eligibility thresholds conventionally include the stated cutoff
    # unless the wording explicitly makes it strict.
    return "gte"


def extract_performance_filter(text: str) -> Tuple[Optional[str], str]:
    """Extract a source vocabulary grade and exact/ordered comparison mode."""
    normalized = normalize_text(text)
    grade = next(
        (candidate for candidate in PERFORMANCE_GRADES if contains_phrase(normalized, normalize_text(candidate))),
        None,
    )
    if not grade:
        return None, "exact"
    if not re.search(
        r"\b(?:assess|assessed|assessment|grade|graded|performance|quality|rated|rating|evaluation|workmanship|completion)\w*\b",
        normalized,
    ):
        # Every grade token is polysemous somewhere in this estate:
        # "outstanding" is also a receivable balance and "good" can be an
        # asset condition. Require explicit assessment semantics.
        return None, "exact"
    if re.search(r"\b(?:at least|or better|or higher|minimum)\b", normalized):
        return grade, "gte"
    if re.search(r"\b(?:above|better than|higher than)\b", normalized):
        return grade, "gt"
    return grade, "exact"


def extract_mean_median_mode(text: str) -> str:
    """Preserve explicit subtraction order; otherwise a gap is non-negative."""
    normalized = normalize_text(text)
    if re.search(r"\bmedian\b.*\b(?:minus|less)\b.*\bmean\b", normalized) or re.search(
        r"\bsubtract\b.*\bmean\b.*\bfrom\b.*\bmedian\b", normalized
    ):
        return "right_minus_left"
    if re.search(r"\bmean\b.*\b(?:minus|less)\b.*\bmedian\b", normalized) or re.search(
        r"\bsubtract\b.*\bmedian\b.*\bfrom\b.*\bmean\b", normalized
    ):
...
class IntentPlanner:
    CLIENT_SCOPED = {
        ...
        "category_aggregate", "doc_filtered_aggregate",
    ...
    def _classify(
        ...
        if performance_grade:
            score["doc_filtered_aggregate"] = 10.5
    ...
    def _compile(self, pattern: str, slots: Dict[str, Any]) -> List[SubTask]:
        ...
        if pattern in sql_patterns:
            ...
            if sql_type == "client_portfolio_values":
                params["difference_mode"] = slots["mean_median_mode"]
            return [
        ...
        if pattern == "doc_filtered_aggregate":
            return [
                SubTask(
                    "T1",
                    "sql_query",
                    "Aggregate projects matching the evidenced performance grade",
                    {
                        "sql_type": "client_grade_projects",
                        **base,
                        "grade": slots["performance_grade"],
                        "comparison": slots["performance_comparison"],
                    },
                ),
                SubTask("T2", "math_compute", "Sum grade-filtered projects", {"metric": "sum_inr"}, ["T1"]),
            ]
        ...
        if pattern == "threshold_aggregate":
            return [
                SubTask("T1", "sql_query", "Aggregate projects meeting threshold", {"sql_type": "client_threshold_projects", **base, "threshold_inr": threshold, "comparison": slots["threshold_comparison"]}),
    ...
    def plan(self, question: str, answer_type: Optional[str] = None) -> ExecutionPlan:
        ...
        if initial_project.ambiguous and client_res.value and client_res.confidence >= 0.82:
            constrained = self.entities.resolve_project(question, client=client_res.value)
            if constrained.value:
                project_res = type(constrained)(
                    constrained.value,
                    min(0.97, constrained.confidence + 0.04),
                    "client_constrained_project",
        ...
        threshold_comparison = extract_threshold_comparison(question)
        performance_grade, performance_comparison = extract_performance_filter(question)
        ...
        pattern, route_conf, alternatives, scores = self._classify(
            question, answer_type, categories, threshold, years, performance_grade
        )
        ...
        slots = {
            ...
            "mean_median_mode": extract_mean_median_mode(question),
        ...
        return ExecutionPlan(
            ...
            extra_params={
                ...
                **({"threshold_comparison": threshold_comparison} if threshold is not None else {}),
                "role": slots["role"],
                **(
                    {
                        "performance_grade": performance_grade,
                ...
                "resolution_sources": {
                    "person": person_res.source,
                    "client": client_res.source,
                    "project": project_res.source,
                },
            },
            subtasks=subtasks,
