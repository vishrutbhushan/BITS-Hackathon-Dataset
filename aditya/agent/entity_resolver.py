...
class EntityResolver:
    ...
    def resolve_project(
        ...
        for project in self.projects:
            ...
            if person and normalize_text(person) != lead:
                continue
            if client and normalize_text(project.get("client") or "") != normalize_text(client):
                continue
    ...
    def resolve_project_dense(
        ...
        if signal.project is not None and client and normalize_text(
            signal.project.get("client") or ""
        ) != normalize_text(client):
            return Resolution(None, 0.0, "dense_project_client_mismatch", signal.alternatives)
