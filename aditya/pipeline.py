...
class BidIntelligencePipeline:
    ...
    def _answer_from_seed(
        ...
        if plan.pattern == "boq_quantity_variance":
            if isinstance(final_answer, (int, float)):
                final_answer = round(float(final_answer), 3)
                if final_answer == 0:
                    final_answer = 0
        elif answer_type == "percent" or plan.pattern in ["referenced_share", "collection_rate"]:
            if isinstance(final_answer, (int, float)):
