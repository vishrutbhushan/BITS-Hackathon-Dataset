...
def parse_performance_grade(text: str) -> Tuple[str, str]:
    """Extract a performance grade only from an evidenced assessment phrase."""
    vocabulary = r"Outstanding|Excellent|Very\s+Good|Satisfactory|Good"
    patterns = (
        rf"3\.\s*Quality\s+Assessment\s*\n+[\s\S]{{0,500}}?\b({vocabulary})\b",
        rf"\b(?:graded|assessed\s+as|overall\s+performance\s+was)\s*[\"'\s]*\b({vocabulary})\b",
        r"\bquality\s+of\s+work\s+has\s+been\s+found\s+(satisfactory)\b",
        r"\btaken\s+over\s+on\s+(satisfactory)\s+completion\b",
    )
    for pattern in patterns:
        match = re.search(pattern, text, re.I | re.S)
        if match:
            raw = re.sub(r"\s+", " ", match.group(1)).strip()
            return raw.title(), match.group(0).strip()
    conforming = re.search(
        r"\bworkmanship\s+conforming\s+to\s+the\s+technical\s+specifications\b",
        text,
        re.I | re.S,
    )
    if conforming:
        return "Good", conforming.group(0).strip()
    return "", ""
...
def parse_client_certificate(text: str, doc_id: str) -> Dict[str, Any]:
    ...
    grade, grade_raw = parse_performance_grade(text)
    return {
        "package_number": package,
        ...
        "project_lead": lead.strip(),
        "role": "",
        "performance_grading": grade,
        "performance_grade_raw": grade_raw,
    }
...
def reconcile_project(
    ...
    priorities = {
        ...
        "role": ("portfolio",),
        "performance_grading": ("client_certificate",),
    }
        ...
        for field in reconciled:
            ...
            yield (
                ...
                (
                    ...
                    if field == "value_inr"
                    else str(record.get("performance_grade_raw", value))
                    if field == "performance_grading"
                    else str(value)
                ),
                str(value),
