...
def run_validation():
    ...
    # 19. Performance grading is an evidenced client-certificate fact. A
    # silent default would corrupt grade-filtered portfolio boundaries.
    missing_grades = con.execute("""
        SELECT COUNT(*) FROM projects
        WHERE performance_grading NOT IN
              ('Outstanding', 'Excellent', 'Very Good', 'Good', 'Satisfactory')
    """).fetchone()[0]
    grade_evidence = con.execute("""
        SELECT COUNT(DISTINCT package_number)
        FROM project_fact_evidence
    ...
    """).fetchone()[0]
    print(f"[INV-19] Evidenced performance grades: {grade_evidence} / 155", end=" -> ")
    assert missing_grades == 0 and grade_evidence == 155
    print("PASSED ✅")
    passed += 1
